...
namespace AtoIndicator
{
    public partial class MainForm
    {
        public bool HandleTradeLine(int nCurIdx, int checkSellIterIdx)
        {
            ...
            {
                if (ea[nCurIdx].myTradeManager.arrBuyedSlots[checkSellIterIdx].fPowerWithFee <= ea[nCurIdx].myTradeManager.arrBuyedSlots[checkSellIterIdx].fBottomPer - ((ea[nCurIdx].myTradeManager.arrBuyedSlots[checkSellIterIdx].nBuyEndTime + ea[nCurIdx].myTradeManager.arrBuyedSlots[checkSellIterIdx].nFixedDelayTime > nSharedTime) ? ea[nCurIdx].myTradeManager.arrBuyedSlots[checkSellIterIdx].fFixedDelayBottomPad : 0)) // 손익률이 손절퍼센트보다 낮으면
                {
                    isSell = true;
